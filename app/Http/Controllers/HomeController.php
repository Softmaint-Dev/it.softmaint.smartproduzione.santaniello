                ...
                if (sizeof($ols) > 0) {
                    ...
                    foreach ($materiale as $m) {
                        if ($m->Tipo != 0) {
                            ...
                            if ($m->Tipo == 2) {
                                $costo = DB::select('SELECT * from ARCostoDBItem Where Cd_AR = \'' . $m->Cd_AR . '\'and Cd_MGEsercizio = YEAR(GETDATE()) and TipoCosto = \'U\'');
                                if (sizeof($costo) > 0) {
            ...
            if (sizeof($attivita_bolle) > 0) {
                ...
                if (sizeof($OLAttivita) > 0) {
                    ...
                    $attivita_bolla->scarto = db::select('SELECT
                        COALESCE(
                        ((SELECT SUM(PRVRMateriale.Consumo) FROM PRVRMateriale WHERE PRVRMateriale.Id_PRVRAttivita in (SELECT Id_PRvrattivita from prvrattivita where id_prblattivita = ' . $id . ') and ((SELECT Cd_ARGruppo1 from ar where ar.cd_AR = prvrmateriale.cd_ar) != \'010\') and ((SELECT Cd_ARGruppo1 from ar where ar.cd_AR = prvrmateriale.cd_ar) != \'011\') and PRVRMateriale.Tipo != 0 and  PRVRMateriale.Tipo != 3 AND PRVRMateriale.NotePrVRMateriale not like \'%CALO%PESO%\')
                        -
