                    ...
                    if (sizeof($attivita_bolle) > 0) {
                        ...
                        if (sizeof($ordini) > 0) {
                            ...
                            if (sizeof($check) <= 0) {
                                ...
                                DB::SELECT('INSERT INTO ARLotto (Cd_AR,Cd_ARLotto,Descrizione,DataScadenza) VALUES (\'' . $articolo->Cd_AR . '\',\'' . $dati['xLotto'] . '\',\'Lotto ' . $dati['xLotto'] . ' Articolo ' . $articolo->Cd_AR . '\',\'' . $data_scadenza . '\')');
                            }
                        }
                    }
                }
...
//                    $quantita = DB::select('SELECT isnull(sum(QtaProdottaUmFase),0) as QtaProdotta from xWPCollo Where NC = 0 and Id_PrVRAttivita IS NULL and IdCodiceAttivita = ' . $attivita_bolla->Id_PrOLAttivita)[0]->QtaProdotta;
    ...
    */
                        ...
                        if (sizeof($ordini) > 0) {
                            $ordine = $ordini[0];
                            $max_lotto = DB::SELECT('SELECT MAX(xLotto) as max from xWPCollo where IdOrdineLavoro = ' . $attivita_bolla->Id_PrOLAttivita);
                            if (strpos($articolo->Cd_AR, 'MIS') !== false) $xlotto = date('dmY', strtotime('now')); else $xlotto = 'Z' . date('d', strtotime('now')) . $mese_lettera;
                            DB::table('PrVrMateriale')->insert(['Id_PrOLAttivita' => $attivita_bolla->Id_PrOLAttivita, 'Cd_AR' => $ordine->Cd_AR, 'Cd_ARLotto' => (sizeof($max_lotto) > 0) ? $max_lotto[0]->max : $xlotto, 'Cd_ARMisura' => $ordine->Cd_ARMisura, 'FattoreToUM1' => $ordine->FattoreToUM1, 'Cd_MG' => '00001', 'Tipo' => 0, 'Id_PRVRAttivita' => $id_attivita, 'Consumo' => -$consumo_tot]);
                        }*/
            ...
            if (isset($dati['fine_lavorazione_si'])) {
                ...
                if (sizeof($attivita_bolle) > 0) {
                    ...
                    if ($quantita > 0) {
                        ...
                        $insert['Fermo'] = 0;
                        $insert['UltimoVR'] = 1;
                        if(isset($insert['xContatoreRisorsa'])) unset($insert['xContatoreRisorsa']);
                        if(isset($insert['xCd_ARMisura'])) unset($insert['xCd_ARMisura']);
                        $id_attivita = DB::table('PRVRAttivita')->insertGetId($insert);
